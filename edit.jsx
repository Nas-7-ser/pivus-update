...
/* ============================================================
 *  <EdImg> — image éditable (stocke une URL comme un texte)
 *  Placeholder tant qu'aucune URL n'est définie.
 * ============================================================ */
function EdImg({ k, alt, className, placeholder }) {
  const ctx = useContext(ContentCtx);
  const src = ctx.get(k, '');
  const editable = ctx.auth && ctx.editing;
  const cls = 'edimg' + (editable ? ' edimg-on' : '') + (className ? ' ' + className : '');

  const change = () => {
    const url = window.prompt("URL de l'image (https://…). Laissez vide pour retirer.", src || '');
    if (url === null) return;
    ctx.set(k, url.trim(), '');
  };

  return (
    <div className={cls} data-ek={k}>
      {src
        ? <img src={src} alt={alt || ''} loading="lazy" />
        : <div className="edimg-ph" aria-hidden="true"><span>{placeholder || 'Photo'}</span></div>}
      {editable && (
        <button type="button" className="edimg-btn" onClick={change}>
          {src ? "Changer l'image" : "Ajouter une image"}
        </button>
      )}
    </div>
  );
...
/* ============================================================
 *  <EdVideo> — vidéo éditable (stocke une URL comme un texte)
 *  Supporte YouTube / Vimeo (iframe) et fichiers .mp4/.webm (<video>).
 *  Placeholder « play » tant qu'aucune URL n'est définie.
 * ============================================================ */
function embedUrl(url) {
  try {
    ...
    if (/\.(mp4|webm|ogg|mov)(\?|$)/i.test(u.pathname)) return { type: 'video', src: u.href };
    return { type: 'iframe', src: u.href };
  } catch (e) { return null; }
}

function EdVideo({ k, poster, className }) {
  const ctx = useContext(ContentCtx);
  const src = ctx.get(k, '');
  const editable = ctx.auth && ctx.editing;
  const cls = 'edvideo' + (editable ? ' edvideo-on' : '') + (className ? ' ' + className : '');
  const posterSrc = ctx.get((k || '') + '.poster', poster || '');

  const change = () => {
    const url = window.prompt('Lien de la vidéo (YouTube, Vimeo ou fichier .mp4). Laissez vide pour retirer.', src || '');
    if (url === null) return;
    ctx.set(k, url.trim(), '');
  };
  const changePoster = () => {
    const url = window.prompt("URL de l'image d'aperçu (miniature) — optionnel.", posterSrc || '');
    if (url === null) return;
    ctx.set((k || '') + '.poster', url.trim(), '');
  };

  const emb = src ? embedUrl(src) : null;

  return (
    <div className={cls} data-ek={k}>
      {emb && emb.type === 'iframe' && (
        <iframe src={emb.src} title="Vidéo de démonstration" frameBorder="0"
          allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
          allowFullScreen></iframe>
      )}
      {emb && emb.type === 'video' && (
        <video src={emb.src} poster={posterSrc || undefined} controls playsInline></video>
      )}
      {!emb && (
        <div className="edvideo-ph" style={posterSrc ? { backgroundImage: 'url(' + posterSrc + ')' } : undefined}>
          <span className="edvideo-play" aria-hidden="true">
            <svg viewBox="0 0 24 24" fill="currentColor"><path d="M8 5v14l11-7z" /></svg>
          </span>
          <span className="edvideo-label">Démo vidéo — bientôt disponible</span>
        </div>
      )}
      {editable && (
        <div className="edvideo-controls">
          <button type="button" className="edimg-btn" onClick={change}>{src ? 'Changer la vidéo' : 'Ajouter une vidéo'}</button>
          <button type="button" className="edimg-btn ghost" onClick={changePoster}>Miniature</button>
        </div>
      )}
    </div>
  );
...
window.PIVUS_EDIT = { ContentProvider, ContentCtx, Ed, EdImg, EdVideo, AdminHost };
})();
